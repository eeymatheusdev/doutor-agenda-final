...
import { upsertDoctorFinance } from "@/actions/doctor-finances";
import { upsertDoctorFinanceSchema } from "@/actions/doctor-finances/schema";
...
const formSchema = upsertDoctorFinanceSchema.omit({ doctorId: true });
...
export default function UpsertFinanceForm({
  ...
  const { execute, isExecuting } = useAction(upsertDoctorFinance, {
    onSuccess: () => {
      toast.success("Lançamento financeiro salvo com sucesso.");
      queryClient.invalidateQueries({
        queryKey: ["doctor-finances", doctorId],
      });
      onSuccess?.();
    },
    onError: () => {
  ...
  return (
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Preencha os detalhes da comissão ou pagamento.
        </DialogDescription>
      </DialogHeader>
      <Form {...form}>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea
                    placeholder="Ex: Comissão de procedimento X, Adiantamento..."
                    {...field}
                    value={field.value ?? ""}
                  />
                </FormControl>
